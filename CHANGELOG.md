...
- **Support for Divided League Structures**: Implemented `_find_team_in_tables` helper to support leagues with Play-off/Play-out categories (e.g., Romanian Liga I).
- **Composite Table Handling**: Sensors now correctly identify and prioritize the specific sub-league table where the tracked team is located.

## [1.8.4] - 2026-02-21

### Fixed

- **Coach Sensor**: Corrected the fallback parsing path for coach attributes (`age`, `country`). Detailed info is occasionally located under `overview.lastLineupStats.coach` rather than `overview.coach`.

## [1.8.3] - 2026-02-21

### Fixed

- **Coach Sensor**: Fixed issue where the coach's `country` attribute was parsing as `N/A`. The extraction logic now properly checks the `overview.coach` JSON block where these details are nested in certain API responses.
...
- **Coach Sensor**: Fixed issue where the coach sensor would return `N/A` for certain teams (like Rapid Bucuresti) due to a parsing path issue in the API response.
